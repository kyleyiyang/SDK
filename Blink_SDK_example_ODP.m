...
if constructed_okay.value ~= 0  % Convention follows that of C function return values: 0 is success, nonzero integer is an error
    ...
    for n = 1:5
        calllib('Blink_C_wrapper', 'Write_overdrive_image', board_number, ImageOne, wait_For_Trigger, external_Pulse, timeout_ms);
        pause(2.0) % This is in seconds
        calllib('Blink_C_wrapper', 'Write_overdrive_image', board_number, ImageTwo, wait_For_Trigger, external_Pulse, timeout_ms);
        pause(2.0) % This is in seconds
    end
